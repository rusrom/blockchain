...
class Blockchain:
    ...
    def broadcast_to_other_nodes(self, data_as_dict):
        ...
        for node in self.__peer_nodes:
            ...
            try:
                ...
                peer_node = response.json()
                if response.ok:
                    result['ok'].append(True)
                    status = 'OK'
                    if 'OLDER STATE' in peer_node["message"]:
                        status = 'Failed'
                    print(f'{node}: Broadcast {data} {status}! {peer_node["message"]}')
    ...
    def add_transaction(self, sender, public_key, signature, recipient, amount, broadcast=True):
        ...
        if self.resolve_conflicts:
            ...
            response['message'] = 'Blockchain has OLDER STATE.'
            return response
